// Package cmd implements the CobraCLI commands for the aiassess CLI. Subcommands for the CLI should all live within
...
import (
	...
	"github.com/Method-Security/aiassess/internal/config"
	"github.com/palantir/pkg/datetime"
	"github.com/palantir/witchcraft-go-logging/wlog/svclog/svc1log"
	"github.com/spf13/cobra"
)

// AIAssess is the main struct for the aiassess CLI. It contains both the root command and all subcommands that can be
...
// NewAIAssess creates a new AIAssess struct with the provided version string. The AIAssess struct is used throughout the
...
func NewAIAssess(version string) *AIAssess {
	aiassess := AIAssess{
		Version: version,
		RootFlags: config.RootFlags{
			Quiet:   false,
...
// InitRootCommand initializes the root command for the aiassess CLI. This command is the parent command for all other
...
func (a *AIAssess) InitRootCommand() {
	...
	a.VersionCmd = &cobra.Command{
		Use:   "version",
		Short: "Prints the version number of aiassess",
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			return nil
		},
